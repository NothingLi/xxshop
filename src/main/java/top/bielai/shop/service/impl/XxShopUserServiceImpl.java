...
@Service
public class XxShopUserServiceImpl implements XxShopUserService {
    ...
    public String login(String loginName, String passwordMD5) {
        ShopUser user = mallUserMapper.selectByLoginNameAndPasswd(loginName, passwordMD5);
        if (user != null) {
            ...
            ShopUserToken mallUserToken = xxShopUserTokenMapper.selectByPrimaryKey(user.getUserId());
            //当前时间
            Date now = new Date();
            //过期时间
            Date expireTime = new Date(now.getTime() + 2 * 24 * 3600 * 1000);//过期时间 48 小时
            if (mallUserToken == null) {
                mallUserToken = new ShopUserToken();
                mallUserToken.setUserId(user.getUserId());
                mallUserToken.setToken(token);
                mallUserToken.setUpdateTime(now);
                mallUserToken.setExpireTime(expireTime);
                //新增一条token数据
                if (xxShopUserTokenMapper.insertSelective(mallUserToken) > 0) {
                    //新增成功后返回
                    return token;
                }
            } else {
                ...
                mallUserToken.setExpireTime(expireTime);
                //更新
                if (xxShopUserTokenMapper.updateByPrimaryKeySelective(mallUserToken) > 0) {
                    //修改成功后返回
                    return token;
                }
            }
    ...
    @Override
    public Boolean updateUserInfo(ShopUserUpdateParam mallUser, Long userId) {
        ShopUser user = mallUserMapper.selectByPrimaryKey(userId);
        if (user == null) {
    ...
    public Boolean logout(Long userId) {
        return xxShopUserTokenMapper.deleteByPrimaryKey(userId) > 0;
    }

    @Override
    public PageResult getXxShopUsersPage(PageQueryUtil pageUtil) {
        List<ShopUser> mallUsers = mallUserMapper.findShopUserList(pageUtil);
        int total = mallUserMapper.getTotalShopUsers(pageUtil);
